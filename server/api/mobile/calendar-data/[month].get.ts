...
export default defineEventHandler(async (event) => {
  try {
    ...
    const mealsQuery = await mealsRef.where("isPublished", "==", true).get();
    ...
    if (!clientQuery.empty) {
      ...
      if (publishedDates.length > 0) {
        ...
        for (let i = 0; i < publishedDates.length; i += 30) {
          ...
          batches.push(
            selectionsRef.where("__name__", "in", selectionIds).get(),
          );
        }
      ...
      // Query cancelled deliveries for this month
      const cancelledRef = firestore.collection("cancelledDeliveries");
      const cancelledQuery = await cancelledRef
        .where("clientId", "==", clientId)
        .get();

      // Filter to dates within this month
      cancelledDates = cancelledQuery.docs
