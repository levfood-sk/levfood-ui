...
const activeTab = ref<'stripe' | 'superfaktura' | 'email' | 'full'>('full')
...
async function testSuperfaktura() {
  ...
  try {
    const response = await $fetch('/api/test/superfaktura-production', {
      method: 'POST',
    })
...
<template>
  <div class="space-y-6">
    ...
    <UTabs v-model="activeTab" :items="[
      { key: 'full', label: 'Full Integration', icon: 'i-lucide-workflow' },
      { key: 'stripe', label: 'Stripe', icon: 'i-lucide-credit-card' },
      { key: 'superfaktura', label: 'Superfaktura', icon: 'i-lucide-file-text' },
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <UCard>
                  ...
                  <div v-if="fullIntegrationResult.results?.stripe" class="text-sm space-y-1">
                    <div><strong>ID:</strong> {{ fullIntegrationResult.results.stripe.paymentIntentId }}</div>
                ...
                <UCard>
                  ...
                  <div v-if="fullIntegrationResult.results?.email" class="text-sm space-y-1">
                    ...
                    <div><strong>ID:</strong> {{ fullIntegrationResult.results.email.messageId?.substring(0, 20) }}...</div>
                  </div>
                </UCard>
              </div>
            </div>
